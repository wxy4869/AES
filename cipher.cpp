#include <cstdio>
#include "xtime.h"

int Nb = 4;  // state 的字数, state 有 4 行 Nb 列
int Nk = 4;  // key 的字数, key 有 4 行 Nk 列
int Nr = 10;  // 轮次

u_int8_t S_box[256] = {
    
};

void AddRoundKey(u_int8_t state[], u_int8_t w[], int r) {
    u_int8_t * wb = &w[4 * Nb * r];  // 轮密钥
    for (int i = 0; i < Nb; i++) {
        for (int j = 0; j < 4; j++) {
            state[i + j * Nb] ^= wb[j + i * 4];  // state 的第 i 列第 j 个字节 ^ 轮密钥中第 i 个字第 j 个字节
        }
    }
}

void SubBytes(u_int8_t state[]) {
    for (int i = 0; i < 4 * Nb; i++) {
        state[i] = S_box[state[i]];
    }
}

void ShiftRows(u_int8_t state[]) {
    /* 0  1  2  3      0  1  2  3
     * 4  5  6  7  ->  5  6  7  4
     * 8  9  10 11     10 11 8  9
     * 12 13 14 15     15 12 13 14
     */
    u_int8_t tmp;
    // 第 0 行不移位
    // 第 1 行左移 1 位
    tmp = state[4]; state[4] = state[5]; state[5] = state[6]; state[6] = state[7]; state[7] = tmp;
    // 第 2 行左移 2 位
    tmp = state[8]; state[8] = state[10]; state[10] = tmp;
    tmp = state[9]; state[9] = state[11]; state[11] = tmp;
    // 第 3 行左移 3 位, 即右移 1 位
    tmp = state[15]; state[15] = state[14]; state[14] = state[13]; state[13] = state[12]; state[12] = tmp;
}

void MixColumns(u_int8_t state[]) {
    unsigned char tmp[Nb];
    for (int i = 0; i < Nb; i++) {
        for (int j = 0; j < 4; j++) {
            tmp[j] = state[i + j * Nb];  // 第 i 列第 j 个字节
        }
        state[i + 0 * Nb] = x2time(tmp[0]) ^ x3time(tmp[1]) ^ tmp[2] ^ tmp[3];
        state[i + 1 * Nb] = tmp[0] ^ x2time(tmp[1]) ^ x3time(tmp[2]) ^ tmp[3];
        state[i + 2 * Nb] = tmp[0] ^ tmp[1] ^ x2time(tmp[2]) ^ x3time(tmp[3]);
        state[i + 3 * Nb] = x3time(tmp[0]) ^ tmp[1] ^ tmp[2] ^ x2time(tmp[3]);
    }
}

void Cipher(const u_int8_t in[], u_int8_t out[], u_int8_t w[]) {
    /* 将输入 in 按规则排列到 state 中
     * 0  1  2  3      0  4  8  12 .
     * 4  5  6  7  ->  1  5  9  13 .
     * 8  9  10 11     2  6  10 14 .
     * 12 13 14 15     3  7  11 15 .
     * .  .  .  .
     * in 为 Nb 行 4 列, state 为 4 行 Nb 列
     */
    u_int8_t state[16];
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < Nb; j++) {
            state[j + i * Nb] = in[i + 4 * j];  // state 的 i 行 j 列为 in 的 j 行 i 列
        }
    }

    // Nr 轮
    AddRoundKey(state, w, 0);
    for (int i = 1; i <= Nr - 1; i++) {
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
        AddRoundKey(state, w, i);
    }
    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, w, Nr);

    // 结束
    for (int i = 0; i < 16; i++) {
        out[i] = state[i];
    }
}
